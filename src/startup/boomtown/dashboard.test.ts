// src/startup/boomtown/dashboard.test.ts
...
Deno.test('mapCaravanStatus returns "running" when tmux session exists', () => {
  ...
  const tmuxSessions = ['startup-pd-001', 'startup-camp-boss'];
...
Deno.test('mapCaravanStatus returns "idle" for open caravan without session', () => {
  ...
  const tmuxSessions = ['startup-camp-boss']; // No session for this caravan
...
Deno.test('mapCaravansToDashboard converts multiple caravans', () => {
  ...
  const tmuxSessions = ['startup-pd-001']; // Only first caravan has active session
...
Deno.test('mapCaravansToDashboard handles empty caravan list', () => {
  const caravans: CaravanInfo[] = [];
  const tmuxSessions = ['startup-camp-boss'];
...
Deno.test('RELOAD_TRIGGER_FILE uses correct path', () => {
  // Should contain 'startup' and 'reload' in the path
  assertStringIncludes(RELOAD_TRIGGER_FILE, 'startup');
...
// ========================================================================
// findStartupPath Tests
// ========================================================================

Deno.test('findStartupPath prefers local binary', async () => {
  // Create a temporary startup binary in cwd
  const testDir = await Deno.makeTempDir({ prefix: 'startup-test-' });
  ...
  try {
    ...
    // Create a fake startup binary
    const localBinary = `${resolvedTestDir}/startup`;
    await Deno.writeTextFile(localBinary, '#!/bin/bash\necho "local startup"');
    await Deno.chmod(localBinary, 0o755);

    // findStartupPath should find the local binary first
    const foundPath = await findStartupPath();
