// src/startup/boomtown/dashboard.ts

/**
 * Dashboard launcher for Startup Boomtown.
...
export const RELOAD_TRIGGER_FILE = '/tmp/startup-boomtown-reload';

/**
 * Find the startup binary path.
 *
 * Checks:
 * 1. ${CWD}/startup (compiled binary in project)
 * 2. startup in PATH
 *
 * @returns Full path to startup binary
 * @throws If startup cannot be found
 */
export async function findStartupPath(): Promise<string> {
  // Try local compiled binary first
  const localPath = join(Deno.cwd(), 'startup');
  try {
  ...
  throw new Error(
    'startup binary not found. Run "deno compile --allow-all --output=startup startup.ts" first.',
  );
}
 ...
 * Returns session names for startup-prefixed sessions.
 */
 ...
 */
async function generateDashboardConfig(startupPath: string): Promise<string> {
  ...
  return await writeMprocsConfig(dashboardCaravans, startupPath);
}
...
export async function launchBoomtown(): Promise<void> {
  console.log('Launching Startup Boomtown dashboard...');

  // Find startup binary path
  let startupPath: string;
  try {
    startupPath = await findStartupPath();
    console.log(`Using startup at: ${startupPath}`);
  } catch (error) {
  ...
  while (true) {
    // Generate config
    const configPath = await generateDashboardConfig(startupPath);
